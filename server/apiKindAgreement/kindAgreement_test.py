...
class TestContratoFuecRest(unittest.TestCase):
    def setUp(self):
        self.domain = 'mi.co'
    ...
    def test_getallkindHiring(self):
        path1 = 'apiFuec/allKindAgreement'  # Only candidate role
        ...
        self.assertEqual(r.status_code, 200, 'Kind Agreement is ok')
        answer_json = json.loads(r.text)
        dict_result = answer_json['result']
        self.assertTrue((type(dict_result) is list), 'Kind Agreement get all')


suite = unittest.TestLoader().loadTestsFromTestCase(TestContratoFuecRest)
